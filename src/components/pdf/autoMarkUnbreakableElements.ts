import {waitForLayout, mmToPx, type PDFConfig} from "./config"
// 改进的自动标记函数
const autoMarkUnbreakableElements = async (container: HTMLElement, config: PDFConfig) => {
    // 等待浏览器完成初始布局
    await waitForLayout();

    // 计算页面高度阈值（A4纸减去边距）
    const pageHeightPx = mmToPx(297 - 2 * config.pageMargin, config);
    ...
    sortedElements.forEach((el, index) => {
      ...
      const elHeight = (height + margin) * config.scaleFactor;
