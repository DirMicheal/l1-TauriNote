...
const calculatePageBreaks = (container: HTMLElement, config: PDFConfig): number[] => {
    const elements = Array.from(container.children);
    const breaks: number[] = [];

    // 计算A4页面可用高度（毫米转像素，考虑DPI和缩放）
    const maxHeightPx = (297 - config.pageMargin * 2) *
                       (config.dpi / 25.4) *
                       config.scaleFactor * 0.92; // 提高阈值到92%，减少内容截断风险
