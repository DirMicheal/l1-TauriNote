import { type PDFConfig, waitForLayout } from "../config";
 ...
 * @returns 渲染后的Canvas
 */
const renderPageToCanvas = async (pageContainer: HTMLElement, _includedElements: Element[], config: PDFConfig): Promise<HTMLCanvasElement> => {
    ...
    ['position', 'left', 'top', 'opacity', 'zIndex', 'visibility'].forEach(prop => {
      originalStyles[prop] = pageContainer.style[prop as keyof CSSStyleDeclaration] as string;
    });
...
.content, .markdown-content {
  ...
  opacity: 1 !important;
    ...
    Object.keys(originalStyles).forEach(prop => {
      (pageContainer.style as unknown as Record<string, string>)[prop] = originalStyles[prop];
    });
