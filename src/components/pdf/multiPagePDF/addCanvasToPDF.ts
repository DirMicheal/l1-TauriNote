 ...
 */
const addCanvasToPDF = async (pdf: jsPDF, canvas: HTMLCanvasElement, pageIndex: number, totalPages: number): Promise<void> => {
    // 添加新页面（首页除外）
    ...
    if (imgHeight > maxAllowedHeight * 1.5) {
      ...
      for (let i = 0; i < pagesNeeded; i++) {
        ...
        if (tempCtx) {
          ...
          if (hasContent) {
            // 获取图像数据并添加到PDF
            const imageData = tempCanvas.toDataURL('image/jpeg', PDF_CONFIG.quality);
            pdf.addImage(
              imageData,
              'JPEG',
      ...
      const xOffset = (PDF_CONFIG.contentWidth - scaledWidth) / 2 + PDF_CONFIG.pageMargin;
  
      pdf.addImage(
        canvas.toDataURL('image/jpeg', PDF_CONFIG.quality),
        'JPEG',
        xOffset,
        PDF_CONFIG.pageMargin,
        scaledWidth,
    ...
    pdf.text(pageText, 210 - PDF_CONFIG.pageMargin - 10, 297 - PDF_CONFIG.pageMargin - 3, { align: 'right' });
  };
