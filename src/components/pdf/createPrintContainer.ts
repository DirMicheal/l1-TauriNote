...
import { getHtmlStyles, getPrintContainerStyle, PRINT_CONTAINER_STYLE, type PDFConfig } from "./config"

// 增强版容器创建函数 - 解决空白PDF问题
const createPrintContainer = async (html: string, config: PDFConfig): Promise<HTMLElement> => {
    ...
    style.textContent = getHtmlStyles(config);

    // 应用容器样式
    Object.assign(container.style, getPrintContainerStyle(config));
    ...
    try {
      ...
      // 自动标记不可分割元素并获取估计页数
      await autoMarkUnbreakableElements(container, config);
